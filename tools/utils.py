...
def save_checkpoint(args, step, model, optimizer, ema_model=None):
    if dist_util.is_main_process():
        ...
        filename = f"{args.model}_{args.mean_type}_{args.path_type}_{step}.pth"
...
def generate_samples(args, step, sampler, save_grid=False):
...
def calculate_metrics(args, **kwargs):
    ...
    step, sampler, evaluator, ref_acts, ref_stats, ref_stats_spatial = (
        kwargs['step'],  kwargs['sampler'], kwargs['evaluator'], 
        kwargs['ref_acts'], kwargs['ref_stats'], kwargs['ref_stats_spatial'])
    
    # Sample images and get the array
    arr = generate_samples(args, step, sampler)
    if dist_util.is_main_process():
